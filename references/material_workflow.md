...
目标：基于小映已审核成稿，协助生成或下载图片/视频素材，并通过小映导入接口写回 `material_plan.json`。

收到用户给出的小映任务目录后，默认只做分析，不要立刻生成图片，不要写入项目文件，不要调用导入 API。
...
这个文件是权威成稿映射，里面的 `lines[].line_id` 是小映原始行号。这个行号可能跳号，不一定是连续的。
...
复制提示词时，小映会根据源文件类型提前写明本次的素材匹配模式：

- `全部匹配`：来源是纯音频。默认每一行成稿都需要画面素材。
- `重点匹配`：来源是视频。默认已有原始画面，只给核心观点、转场、强调句、强画面感内容补充素材。

这个模式已经由小映决定，Agent 不要再询问用户“全部匹配还是部分匹配”。如果用户后续主动要求改成另一种模式，再按用户的新要求执行。
...
如果用户根据建议想调整文案、换行、合并行或拆分行，必须提示用户回到小映前端界面修改。小映前端会自动保存最新成稿到：
...
3. 根据小映提示词指定的素材匹配模式，以及用户确认后的具体行清单生成图片或视频素材。
4. 素材必须保存到当前小映任务目录内部。
...
- 支持分批生成。用户如果只要求先生成 2-3 张样张，生成完这一批就立刻写结果文件并调用小映导入 API。
- 不要等全部图片都生成完才导入。
- 小映后端会合并写入 `material_plan.json`，不要覆盖已有素材。
...
- `type` 可以是 `image` 或 `video`；不填时小映会按文件扩展名推断。
