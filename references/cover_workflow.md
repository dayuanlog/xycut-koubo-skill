...
目标：把已经生成好的封面图片接入小映项目。封面图片的创意、构图、人物 IP、尺寸和生成方式，交给 `dayuan-ip` 或用户指定的其他生图 skill 处理；xycut-koubo-skill 不负责设计封面图，只负责把最终图片登记到小映。

## D1. 先确认封面图已经生成

封面图必须先由其他生图 skill 或用户手动准备好，并保存到当前小映任务目录内部，例如：

```text
<task_dir>/cover/agent_cover_001.png
```

不要把封面图保存到 `C:/Users/Administrator/.codex/skills/xycut-koubo-skill` 目录，也不要只返回外部 URL。小映需要本地文件路径，刷新页面后才能稳定预览和生成草稿。
...
封面标题不是封面图片路径的一部分。不要只把标题写进 `cover_plan.json` 的 `title` 字段，否则小映前端标题文本框可能不会展示。
...
用户刷新小映后，标题文本会显示在「草稿生成」页的标题文本框中。
...
用户刷新页面后，可以在「草稿生成」页的封面模块看到封面预览。后续点击生成草稿时，小映会自动启用该封面。
